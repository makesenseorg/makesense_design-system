...
import MksList from "@@/components/List";
...
export default {
  ...
  components: { DsList, DsListItem, MksList, VRuntimeTemplate },
  data() {
    return {
      md: new markdownIt()
        .use(emoji)
  ...
  methods: {
    parseComponent() {
      ...
      // console.log('template', `<div>${componentHtml}</div>`);
      return `<div>${componentHtml}</div>`;
      /*
      ...
      console.log("comp", typeof component);
      // return component;

      return component; // "parsed-markdown-component";
...
/*
  render() {
    console.log("parsed", this.parsedComponent);
    return this.parsedComponent ? this.parsedComponent() : {};
  },
*/
};
</script>
