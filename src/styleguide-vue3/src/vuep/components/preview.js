...
export default {
  ...
  render() {
    this.className = "vuep-scoped-" + generateUID();
  ...
  methods: {
    renderCode() {
      ...
      if (this.codeVM) {
        this.codeVM.unmount();
        container.removeChild(this.codeEl);
      }
      ...
      try {
        const parent = this;
        this.codeVM = createApp({
          parent,
          ...val,
        });
        this.codeVM.config.compilerOptions.whitespace = "preserve";
        this.codeVM.use(router);
        this.codeVM.use(DS, {});
        this.codeVM.mixin({
          methods: {
            $date: dayjs,
        ...
        Object.values(dsComponents).forEach((component) => {
          this.codeVM.component(component.name, component);
        });
        this.codeVM.mount(this.codeEl);
